      PROGRAM INTPT
c     f77 -o intpt.x intpt.f
C     downloaded from http://earth-info.nga.mil/GandG/wgs84/gravitymod/egm96/egm96.html
...
c      open(11, file= '/tmp/intpt.inp', status='old')
      open(11, file= 'tmp/intpt_input.txt', status='old')
c output file of interpolated values
c      open(20, file='/tmp/intpt.out')
      open(20, file='tmp/intpt_output.txt')
...
c      IF(K.NE.1.AND.K.NE.KMAX(J)) WRITE(IUN6,7001) K-1,XMI,XMA,PROZ,
c     1 N(K,J),(IZEI(L),L=1,40)
c      IF(K.EQ.1) WRITE(IUN6,7002) K-1,XMA,PROZ,N(K,J),(IZEI(L),L=1,40)
c      IF(K.EQ.KMAX(J)) WRITE(IUN6,7004) K-1,XMI,PROZ,N(K,J),
c     1 (IZEI(L),L=1,40)
  320 CONTINUE
c  370 WRITE(IUN6,7005)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
