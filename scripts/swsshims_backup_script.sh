...
#######################################################################
#                                                                     #
#        NB: this script is not intended to be run on it's            #
#           own, but rather to be called by the following:            #
#                                                                     #
...
#   Assemble a name and path for the tar file to be created.
    tar_name="/project/obr/swsshims/tar/"$current_flight_name".tar.bz"
#   Tar up the current flight
    tar -cjf $tar_name $current_flight_dir > /dev/null
    if [ $? -eq 0 ]
     then
#     Set permissions and give ownership to swsshims
      chmod 664 $tar_name > /dev/null
      #chown swsshims $tar_name > /dev/null <---command now barred
#     Pass to MASS
      echo -e "\n--- Moosing flight "$current_flight_name" ---"
#     Put the file into MASS - MOOSE will reject if file already exists in MASS
      moo put -v $tar_name moose:/adhoc/projects/146data/swsshims/rawdata/
...
# List any flights which failed to tar.
  if [ "$did_not_tar" != "" ]
   then
    echo -e "\n--------------------------------------------------------------------------\n"
    did_not_tar=" \nThe following flights did not tar properly and have not been \
      archived:\n"$did_not_tar"\n"
    wait $!
    echo -e $did_not_tar
  fi
# Confess any flights which did not go to MASS.
  ...
  echo ""
  date +"                            %H%M %A %d/%m/%y"
  echo -e "\n======================== FINISHED SWS/SHIMS ARCHIVE RUN ========================"
  echo ""
# Email the archive log
  mailx -s 'SWS/SHIMS archive log' -b $script_run_by, $owner < $log_name
  mass_list_name=`date +"/home/h03/swsshims/mass_listing_%y%m%d_%H%M.txt"`
  touch $mass_list_name
  moo ls -l -v moose:/adhoc/projects/146data/swsshims/rawdata/ >$mass_list_name
  sed 's/:/\ /g' $mass_list_name > /tmp/swsshims_mass_list.tmp
  awk 'BEGIN{ OFS="\t"};{print $5,$6,$7$8,$11":"$12}' /tmp/swsshims_mass_list.tmp >  $mass_list_name
